...
use bevy::prelude::{Added, Color, Commands, Component, Entity, Gizmos, Query, Res, ResMut, Resource, Time, Timer, Transform, Vec2, Without};
...
pub fn set_level_bounds(
    ...
    for (project_entity, ldtk_handle, transform) in levels.iter() {
        if let Some(ldtk_asset) = ldtk_assets.get(ldtk_handle) {
            let level = &ldtk_asset.data().find_raw_level_by_level_selection(&LevelSelection::index(1)).unwrap(); // Access the first level, adjust if needed
            ...
            commands.entity(project_entity).insert(LevelBounds {
                min: Vec2::new(transform.translation.x + padding, transform.translation.y + padding),
                max: Vec2::new(level_width_meters + transform.translation.x - padding, level_height_meters + transform.translation.y - padding),
            });
        }
    }
}

pub fn draw_level_bounds(
    mut gizmos: Gizmos,
...
pub fn enemy_spawn_cycle(
    ...
    if count < round_time.timer.elapsed().as_secs() as usize {
        // spawn_enemy(1, _commands, atlases, rng);
        spawner.spawn(Object::Enemy, EnemySpawnData { enemy_id: "bat".to_string(), player_position: transform.translation.xy(), bounds: bounds.clone() });
        spawner.spawn(Object::Enemy, EnemySpawnData { enemy_id: "zombie".to_string(), player_position: transform.translation.xy(), bounds: bounds.clone()  });
...
pub fn move_player_to_spawn_point(
    ...
    for (entity, _, spawn) in spawn_point.iter() {
        transform.translation = Vec2::new(spawn.translation.x as f32, spawn.translation.y as f32).extend(PLAYER_LAYER);
